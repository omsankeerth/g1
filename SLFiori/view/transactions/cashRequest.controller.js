sap.ui.controller("SLFiori.view.transactions.cashRequest", {
	...
	handleLocationList: function(oEvent) {
		...
		if (FreezeEntry === "" || SubCode === "") {
			sap.m.MessageToast.show("Please select Freezing number & Paying subsidary");
		} else {
			$.ajax({
				//url: "OData/CostCenter.xsodata/CostCenter?$filter=U_SubCode eq '" + SubLoc + 	"'",
				...
				success: function(oData, oResponse) {
					...
					for (var k = 0; k < oMtxRow.length; k++) {
						...
						if (oMtxRow[k].U_Accpt === true) {
						      //var tdCell= document.getElementById("U_Accpt");
                            //var inputF= tdCell.getElementsByTagName("input");
							//this.byId("PrcCode").setEnabled(true);
							//this.oModel("/RequestForm/" + k + "/U_Accpt").editable = false;
							//oMtxRow[j].editable = false;
							 //inputF[0].readOnly = true;
						} else {
							oMtxRow[j].U_Accpt.setEnabled(true);
						}
					}
